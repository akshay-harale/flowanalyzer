...
if __name__ == "__main__":
    ...
    for message in consumer:
        ...
        flow_packet_contents = message.value
        ...
        # Something went wrong unpacking the header, bail out
            ...
            logging.warning(
                "Failed unpacking flow header from " + str("") + " - " + str(flow_header_error))
            continue

        # Check Netflow version
        if int(packet["netflow_version"]) != 9:
            logging.warning("Received a non-Netflow v9 packet from " + str("") + " - SKIPPING PACKET")
            continue  # Bail out
        ...
        while True:  # Iterate through all flows in the packet
            ...
            try:
                ...
                logging.info(
                    "Found flow ID " + str(flow_set_id) + ", length " + str(flow_set_length) + " at " + str(pointer))
            except:
            ...
            if flow_set_id == 0:  # Template flowset
                ...
                parsed_templates = netflow_v9_parser.template_flowset_parse(flow_packet_contents, "",
                                                                            pointer, flow_set_length)  # Parse templates
                ...
                option_templates = netflow_v9_parser.option_template_parse(flow_packet_contents, "",
                                                                           pointer)
            ...
            # Flow data set
                ...
                hashed_id = hash(str("") + str(flow_set_id))

                ### Missing template, drop the flow ###
                if hashed_id not in template_list:
                    logging.warning("Missing template for set " + str(flow_set_id) + " from " + str(
                        "") + ", sequence " + str(packet["sequence_number"]) + " - DROPPING")
                ...
                if template_list[hashed_id]["Type"] == "Flow Data":
                    ...
                    while data_position + 4 <= (flow_set_length + (pointer - 4)):
                        ...
                        logging.info(
                            "Data flow number " + str(flow_counter) + ", set ID " + str(flow_set_id) + " from " + str(
                                ""))
                        ...
                        for template_key, field_size in template_list[hashed_id]["Definitions"].iteritems():
                            ...
                            try:
                                ...
                                logging.info("Skipping undefined field (template_key,field_size) - " + str(
                                    (template_key, field_size)))
                            ...
                            ### Integer field ###
                                ...
                                flow_payload = int_un.integer_unpack(flow_packet_contents, data_position,
                                                                     field_size)  # Unpack the integer
                            ...
                            ### Something Else ###
                                ...
                                logging.warning("Unsupported field number " + str(template_key) + ", size " + str(
                                    field_size) + " from " + str("") + " in sequence " + str(
                                    packet["sequence_number"]))
                            ...
                            if "Options" in v9_fields[
                                template_key]:  # Integer fields with pre-defined values in the v9 standard
                                try:
                                    flow_index["_source"][v9_fields[template_key]["Index ID"]] = \
                                        v9_fields[template_key]['Options'][int(flow_payload)]
                                except Exception as option_warning:
                                    logging.warning("Failed to parse human option, template key " + str(
                                        template_key) + ", option key " + str(flow_payload) + ", from " + str(
                                        "") + " - USING INTEGER VALUE")
                        ...
                        # Non-transport protocols eg OSPF, VRRP, etc
                            ...
                            try:
                                flow_index["_source"][
                                    "Traffic Category"] = ports_protocols_parser.protocol_traffic_category(
                        ...
                        if dns is True:
                            ...
                            if "IPv4 Destination" in flow_index["_source"]:
                                destination_lookups = name_lookups.ip_names(4,
                                                                            flow_index["_source"]["IPv4 Destination"])
                            elif "IPv6 Destination" in flow_index["_source"]:
                                destination_lookups = name_lookups.ip_names(6,
                                                                            flow_index["_source"]["IPv6 Destination"])
                            ...
                            try:  # Pick unique domain Content != "Uncategorized"
                                unique_content = [category for category in src_dest_categories if
                ...
                ### Options Template ###
                    ...
                    logging.info("Creating Netflow v9 Options flow number " + str(flow_counter) + ", set ID " + str(
                        flow_set_id) + " from " + str(""))
            ...
            # Rcvd a flow set ID we haven't accounted for
                ...
                logging.warning("Unknown flow " + str(flow_set_id) + " from " + str("") + " - FAIL")
                pointer = (flow_set_length + pointer) - 4
                flow_counter += 1
                continue
        ...
        if record_num >= bulk_insert_count:
            ...
            dump = json.dumps(flow_dic[0])
            producer.send('netflow9',dump)
