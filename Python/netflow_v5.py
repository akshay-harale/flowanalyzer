...
try:
    ...
    logging.critical("Could not open or bind a socket on port " + str(netflow_v5_port))
...
if __name__ == "__main__":
    ...
    for message in consumer:
        ...
        # Unpack the header
        try:
            logging.info("Unpacking header from " + str(""))
            packet_keys = ["netflow_version", "flow_count", "sys_uptime", "unix_secs", "unix_nsecs", "flow_seq",
                           "engine_type", "engine_id"]  # Netflow v5 packet fields
            packet_values = struct.unpack('!HHIIIIBB', flow_packet_contents[
                                                       0:22])  # Version of NF packet and count of Flows in packet
            packet_contents = dict(zip(packet_keys, packet_values))  # v5 packet fields and values
            logging.info(str(packet_contents))
            logging.info("Finished unpacking header from " + str(""))

        # Failed to unpack the header
        except Exception as flow_header_error:
            logging.warning("Failed unpacking header from " + str("") + " - " + str(flow_header_error))
            continue
        ...
        for flow_num in range(0, packet_contents["flow_count"]):
            ...
            try:
                ...
                logging.warning(
                    "Unknown protocol number - " + str(protocol_num) + ". Please report to the author for inclusion.")
            ...
            if record_num >= bulk_insert_count:
                ...
                for x in flow_dic:
                    dump = json.dumps(x)
                    future = producer.send('netflow5', dump)
